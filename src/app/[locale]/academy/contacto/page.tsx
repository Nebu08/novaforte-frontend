...
export default function AcademyContactoPage() {
  const t = useTranslations("Academy.form");
  ...
  const handleBlur = (field: string, value: string) => {
    ...
    if (field === "role") isError = !value;
    if (field === "students") isError = !value;
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f7f5f4] dark:bg-dark-900 transition-colors duration-300">
      ...
      {/* HERO / HEADER — Con cuadrícula e identidad dorada Academy */}
      <section className="relative min-h-[30vh] flex items-center overflow-hidden bg-dark-900 py-12">
        ...
        <div aria-hidden="true" className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-accent-500 via-accent-600 to-transparent" />
        ...
        <div className="relative max-w-4xl mx-auto px-6 w-full text-center flex flex-col items-center gap-3">
          ...
          >
            ...
            <span>{locale === "es" ? "Volver a Novaforte Academy" : "Back to Academy Division"}</span>
          </Link>
          <h1
        ...
        <div className="relative w-full overflow-hidden bg-white dark:bg-dark-800 border border-gray-200/80 dark:border-gray-700/60 rounded-3xl p-6 sm:p-10 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="relative flex flex-col gap-5 w-full">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {touched.name && errors.name && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "El nombre del coordinador es obligatorio." : "Coordinator's name is required."}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {touched.email && errors.email && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "Ingrese un correo institucional válido." : "Please enter a valid school email."}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {touched.institution && errors.institution && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "La institución educativa es obligatoria." : "Educational institution is required."}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full px-4 py-3 bg-gray-50 dark:bg-dark-950 border rounded-xl text-sm transition-all duration-300 text-dark-900 dark:text-white appearance-none focus:outline-none focus:bg-white dark:focus:bg-dark-900 focus:ring-4 ${
                ...
                >
                  ...
                  <option value="director" className="dark:bg-dark-800">{t("roleOptions.director")}</option>
                  <option value="coordinator" className="dark:bg-dark-800">{t("roleOptions.coordinator")}</option>
                  <option value="teacher" className="dark:bg-dark-800">{t("roleOptions.teacher")}</option>
                  <option value="other" className="dark:bg-dark-800">{t("roleOptions.other")}</option>
                </select>
              ...
              {touched.role && errors.role && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "Seleccione su cargo en la institución." : "Please select your role."}
                </span>
              )}
            </div>

            {/* Número de estudiantes (Rango) */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex justify-between">
                <span>{t("fieldStudents")}</span>
                <span className="text-red-500 font-bold" aria-hidden="true">*</span>
              </label>
              <div className="relative">
                <select
                ...
                >
                  <option value="" className="dark:bg-dark-800">-- Seleccionar --</option>
                  <option value="low" className="dark:bg-dark-800">{locale === "es" ? "Menos de 200" : "Less than 200"}</option>
                  <option value="mid" className="dark:bg-dark-800">{locale === "es" ? "200 a 500" : "200 to 500"}</option>
                  <option value="high" className="dark:bg-dark-800">{locale === "es" ? "Más de 500" : "More than 500"}</option>
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-400">
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" />
              ...
              {touched.students && errors.students && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "Seleccione el rango de estudiantes." : "Please select the range of students."}
                </span>
              )}
            </div>
            ...
            >
              {isSubmitting ? (
                <>
                  ...
                  <span>{locale === "es" ? "Enviando registro..." : "Submitting registration..."}</span>
                </>
              ) : (
                <>
                  <span>{t("submit")}</span>
