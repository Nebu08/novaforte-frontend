...
export default function VetCotizarPage() {
  const t = useTranslations("Vet.form");
  ...
  const handleBlur = (field: string, value: string) => {
    ...
    if (field === "petDetails") isError = !value.trim();
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f7f5f4] dark:bg-dark-900 transition-colors duration-300">
      ...
      {/* HERO / HEADER — Con cuadrícula e identidad Vet emocional */}
      <section className="relative min-h-[30vh] flex items-center overflow-hidden bg-dark-900 py-12">
        ...
        <div aria-hidden="true" className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-pink-700 via-primary-500 to-transparent" />
        ...
        <div className="relative max-w-4xl mx-auto px-6 w-full text-center flex flex-col items-center gap-3">
          ...
          >
            ...
            <span>{locale === "es" ? "Volver a División Vet" : "Back to Vet Division"}</span>
          </Link>
          <h1
        ...
        <div className="relative w-full overflow-hidden bg-white dark:bg-dark-800 border border-gray-200/80 dark:border-gray-700/60 rounded-3xl p-6 sm:p-10 shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="relative flex flex-col gap-5 w-full">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {touched.name && errors.name && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "El nombre del propietario o veterinario es obligatorio." : "Owner or veterinarian name is required."}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {touched.email && errors.email && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "Ingrese un correo electrónico válido." : "Please enter a valid email address."}
                </span>
              )}
            </div>

            {/* Especie / Raza de Mascota */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex justify-between">
              ...
              <input
                ...
                onBlur={(e) => handleBlur("petDetails", e.target.value)}
                placeholder={t("placeholderInstitution")}
                className={`w-full px-4 py-3 bg-gray-50 dark:bg-dark-900/40 border rounded-xl text-sm transition-all duration-300 text-dark-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-655 focus:outline-none focus:bg-white dark:focus:bg-dark-900 focus:ring-4 ${
                  touched.petDetails && errors.petDetails
                    ? "border-red-500/50 focus:border-red-500 focus:ring-red-500/10"
                    : "border-gray-200 dark:border-gray-700/60 focus:border-primary-500 focus:ring-primary-500/10"
                }`}
              />
              {touched.petDetails && errors.petDetails && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "La especie y raza de la mascota es obligatoria." : "Pet species and breed is required."}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option value="wheelchair" className="dark:bg-dark-800">{locale === "es" ? "Silla de Ruedas Adaptativa" : "Adaptive Wheelchair"}</option>
                  <option value="orthosis" className="dark:bg-dark-800">{locale === "es" ? "Órtesis de Codo / Férula de Soporte" : "Elbow Orthosis / Support Splint"}</option>
                  <option value="prosthesis" className="dark:bg-dark-800">{locale === "es" ? "Prótesis de Extremidad" : "Prosthetic Limb"}</option>
                  <option value="other" className="dark:bg-dark-800">{locale === "es" ? "Otro requerimiento" : "Other requirement"}</option>
                </select>
              ...
              {touched.deviceType && errors.deviceType && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "Seleccione un tipo de dispositivo solicitado." : "Please select a device type."}
                </span>
              )}
            </div>

            {/* Mensaje / Comentarios de Movilidad */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex justify-between">
              ...
              {touched.message && errors.message && (
                <span className="text-[11px] font-semibold text-red-500 mt-1 flex items-center gap-1">
                  ⚠️ {locale === "es" ? "Detalle los comentarios clínicos con al menos 10 caracteres." : "Provide mobility comments (minimum 10 characters)."}
                </span>
              )}
            </div>
